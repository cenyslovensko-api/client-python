...
from cenyslovensko_client.config import RPC_SERVER_BIN_ENV
...
class CommandResolverTests(unittest.TestCase):
    ...
    def test_returns_path_binary_when_available(self) -> None:
        with patch.dict(os.environ, {}, clear=True):
            with patch(
                "cenyslovensko_client.adapters.command_resolver.Path.exists", return_value=False
            ):
                with patch("cenyslovensko_client.adapters.command_resolver.shutil.which", return_value="/usr/local/bin/cenyslovensko_rpc_server"):
                    command = resolve_rpc_server_command(None)
    ...
    def test_raises_when_binary_not_found(self) -> None:
        with patch.dict(os.environ, {}, clear=True):
            with patch(
                ...
                with patch("cenyslovensko_client.adapters.command_resolver.shutil.which", return_value=None):
